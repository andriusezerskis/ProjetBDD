...
def insert_medecins(conn, file_path, node_name):
    ...
    for medecin in root.findall('medecin'):
        inami_m = get_text(medecin.find('inami_m'))
...
def insert_pharmaciens(conn, file_path, node_name):
    ...
    for pharmacien in root.findall('pharmacien'):
        inami_p = get_text(pharmacien.find('inami_p'))
...
def insert_specialites(conn, file_path, node_name):
    ...
    for specialite in root.findall('specialite'):
        ...
        # Pas besoin d'utiliser get_text ici
        medicaments = specialite.findall('medicament')
        medicament_systeme_anatomique = ''
        for medicament in medicaments:
            # Utilisez get_text pour lire le texte de chaque élément medicament
            medicament_systeme_anatomique += get_text(medicament) + ';'
        # Supprime le dernier ';'
...
def insert_prescriptions(conn, file_path):
    with open(file_path, 'r', encoding='utf-8') as csvfile:
        ...
        for row in reader:
            ...
            insert_prescription(conn, NISS_patient, inami_medecin, inami_pharmacien,
                                medicament_nom_commercial, date_prescription, duree_traitement)
...
def insert_prescription(conn, NISS_patient, inami_medecin, inami_pharmacien, medicament_nom_commercial, date_prescription, duree_traitement):
    with conn.cursor() as cur:
        ...
        cur.execute("SELECT id FROM medicament WHERE nom_commercial = %s",
        ...
        cur.execute(
            ...
            (NISS_patient, inami_medecin, inami_pharmacien,
             medicament_id, date_prescription, duree_traitement)
        )
        conn.commit()
